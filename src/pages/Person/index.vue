<template>
  <div class="person-Wrapper clearfix">
    <div class="content">
      <div class="person-info">
        <div class="avatar">
          <img v-lazy="userDetail.profile?.avatarUrl" alt="" />
        </div>
        <div class="info">
          <h3 class="nickname">
            {{ userDetail.profile?.nickname }}
          </h3>
          <div class="level clearfix">
            <template v-for="(a, index) in userDetail.profile?.allAuthTypes">
              <!-- 防止desc为空串 -->
              <div class="allAuthTypes" :key="index" v-if="a.desc">
                <img
                  v-lazy="userDetail.profile?.avatarDetail?.identityIconUrl"
                  alt=""
                  :key="index"
                />
                {{ a.desc }}
              </div>
            </template>

            <p>Lv{{ userDetail.level }}</p>
            <i
              class="el-icon-female"
              v-if="userDetail.profile?.gender === 2"
            ></i>
            <i class="el-icon-male" v-if="userDetail.profile?.gender === 1"></i>

            <div
              class="operation"
              v-if="$store.state.user.uid == $route.query.id"
            >
              <button class="edit" @click="loginOut">
                <i class="el-icon-switch-button"></i>
                退出登录
              </button>
            </div>

            <div class="operation" v-else>
              <button class="message">
                <i class="el-icon-message"></i>
                发私信
              </button>
              <button class="plus">
                <i class="el-icon-plus"></i>
                关注
              </button>
            </div>
          </div>
          <ul class="f-cb clearfix">
            <li>
              <a href="javascript:;">
                <em id="event_count">{{ userDetail.profile?.eventCount }}</em>
                <i>动态</i>
              </a>
            </li>
            <li>
              <a href="javascript:;">
                <em id="event_count">{{ userDetail.profile?.follows }}</em>
                <i>关注</i>
              </a>
            </li>
            <li>
              <a href="javascript:;">
                <em id="event_count">{{ userDetail.profile?.followeds }}</em>
                <i>粉丝</i>
              </a>
            </li>
          </ul>
          <div class="des">
            个人介绍:
            <span>{{
              userDetail.profile?.signature
                ? userDetail.profile?.signature
                : "暂无介绍"
            }}</span>
          </div>
        </div>
      </div>

      <HomeNav
        :navBarData="navData"
        :fiexdTop="false"
        @clickBarItem="clickBarItem"
      >
      </HomeNav>

      <div class="person-playlist">
        <router-view
          :createList="createList"
          :collectionList="collectionList"
        ></router-view>
      </div>
    </div>
  </div>
</template>

<script>
import HomeNav from "@/components/HomeNav";
export default {
  name: "Person",
  data() {
    return {
      // 导航的数据
      navData: [
        { name: "创建的歌单", path: "/person/createPlaylist" },
        { name: "收藏的歌单", path: "/person/collection" },
        { name: "创建的音乐专栏", path: "/person/column" },
      ],
      // 用户详情
      userDetail: {},
      // 用户歌单
      playlist: [],
      // 创建的歌单
      createList: [],
      // 收藏的歌单
      collectionList: [],
    };
  },
  mounted() {
    this.getUser_detail(this.$route.query.id);
    this.getUser_playlist(this.$route.query.id);
    this.reqUser_dj(this.$route.query.id);
  },
  methods: {
    // 获取歌手详情
    async getArtistDetail(id) {
      const result = await this.$API.singer.reqArtistDetail(id);
      if (result.code === 200) {
        // this.userDetail = result.data.user;
        console.log(this.userDetail);
      }
    },

    // 获取用户歌单
    async getUser_playlist(id) {
      const result = await this.$API.user.reqUser_playlist(id);
      if (result.code === 200) {
        console.log(result);
        let { id } = this.$route.query;
        result.playlist.forEach((element) => {
          if (element.userId == id) {
            this.createList.push(element);
          } else {
            this.collectionList.push(element);
          }
        });
      }
    },

    // 获取用户详情
    async getUser_detail(id) {
      const result = await this.$API.user.reqUser_detail(id);
      if (result.code === 200) {
        this.userDetail = result;
      }
    },

    // 获取用户电台
    async reqUser_dj(id) {
      const result = await this.$API.user.reqUser_dj(id);
      console.log(result);
    },

    // 退出登录
    loginOut() {
      this.$store.dispatch("user/logoOut");
      // 回到首页
      this.$router.replace({ path: "/discover/recommend" });
    },

    //跳转路由
    clickBarItem({ path }) {
      this.$router.push({ path, query: { id: this.$route.query.id } });
    },
  },

  components: { HomeNav },
};
</script>

<style lang="less" scoped>
.person-Wrapper {
  height: calc(100vh - 130px);
  overflow: hidden;
  overflow-y: scroll;
  .content {
    margin: 0 30px;
    .person-info {
      margin-top: 30px;
      display: flex;
      justify-content: space-between;
      .avatar {
        img {
          width: 180px;
          height: 180px;
          border-radius: 50%;
          border: 1px solid @border-color;
        }
      }
      .info {
        width: calc(100% - 200px);
        text-align: left;
        h3 {
          margin: 0;
        }
        .level {
          margin-top: 10px;
          padding-bottom: 5px;
          border-bottom: 1px solid @border-color;
          .allAuthTypes {
            display: inline-block;
            font-size: 12px;
            vertical-align: top;
            background-color: rgb(255, 243, 220);
            padding: 2px;
            padding-right: 8px;
            border-radius: 50px;
            color: rgb(255, 176, 20);
            margin-right: 5px;
            img {
              width: 15px;
              height: 15px;
              vertical-align: top;
            }
          }
          p {
            margin: 0;
            display: inline-block;
            font-size: 12px;
            background-color: @bg-deep-color-hover;
            padding: 3px 7px;
            border-radius: 50px;
            transform: scale(0.9);
          }
          .operation {
            float: right;
            button {
              border-radius: 50px;
              border: 1px solid @border-color;
              padding: 5px 10px;
              margin-left: 5px;
              background-color: transparent;
              cursor: pointer;
              i {
                margin-right: 5px;
              }
              &:hover {
                background-color: @bg-deep-color-hover;
              }
            }
            .plus {
              i {
                color: rgb(236, 65, 65);
              }
            }
          }
        }
        .f-cb {
          margin-top: 20px;
          li {
            float: left;
            text-align: center;
            a {
              color: #333;
              em {
                display: block;
                font-style: normal;
              }
              i {
                font-style: normal;
                color: @minor-color;
                font-size: 12px;
              }
            }
            &:nth-child(even) {
              border-left: 1px solid @border-color;
              border-right: 1px solid @border-color;
              box-sizing: border-box;
              width: 80px;
              text-align: center;
              margin: 0 30px;
            }
          }
        }
        .des {
          font-size: 12px;
          margin-top: 10px;
          span {
            color: @minor-color;
          }
        }
      }
    }
  }
}
</style>
