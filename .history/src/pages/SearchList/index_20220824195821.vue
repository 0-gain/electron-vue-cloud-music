...
export default {
  ...
  mounted() {
    this.getSearchMultimatch(this.$route.query.keywords);
    console.log('挂载')
  },
  inject:['reload'],
  methods: {
  ...
  watch:{
    ...
    '$route'(){
      // 判断当前路由得query参数是不是与searchWords相同，如果不同，则刷新页面，[当前路径，路由回退和前进页面不刷新]
      if(this.$store.state.search.keywords != this.$route.query.keywords) this.reload()
    }
  },
