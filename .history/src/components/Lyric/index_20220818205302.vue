...
export default {
  ...
  watch: {
    ...
    "$store.state.playList.currentTime"(currentTime) {
      ...
      // 当前索引的歌词高度 = 歌词框的offsetTop(lyric-list)
      let itemHeight = this.$refs.lyricWrap.offsetTop - lyricLine[1].offsetTop
      console.log(this.$refs.lyricWrap.offsetTop,lyricLine[1].offsetTop,itemHeight)
      /* if (lyricIndex < this.lyricList.length) {
        if (lyricLine[lyricIndex - 1].offsetTop > 150) {
          let top = lyricLine[lyricIndex - 1].offsetTop - 150;
